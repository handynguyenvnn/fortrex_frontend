...
import { createAccessToken, login } from 'services';
...
const Login = () => {
  ...
  const [twoFACodeVisible, setTwoFACodeVisible] = useState(true);
  const onClickRegister = () => {
    history.push(PAGE_PATHS.REGISTER);
  };
  const onClickForgotPwd = () => {
    history.push(PAGE_PATHS.FORGOT_PASSWORD);
  };
      ...
      login({ params })
        .then(res => {
          if (res && res.data.StatusCode === 401 || res.data.StatusCode === 400) {
            ...
            setTwoFACodeVisible(false);
          }
           else if (res && res.data.StatusCode === 200) {
            //openNotificationWithIcon('success', 'Notification', res.data.Meg);
            const token = res.data.Reply.Token;
            const username = res.data.Reply.Username;
            _setCookie('token', token);
            _setCookie('username', username);
            _setCookie('fullName', res.data.Reply.Fullname);
            _setCookie('email', res.data.Reply.Email);
            createAccessToken({ Token: token }).then(res => {
              _setCookie('access_token', res.data.access_token);
              history.push(PAGE_PATHS.TRADING);
            });
  ...
  const [, setTypeInput] = useState('text');
  ...
  return (
    <div className="login">
      <div className="login-inner">
        ...
        <p className="title-sub">Don’t have an account? <a onClick={onClickRegister}>Register</a></p>
        <div className="login-form">
          <Form form={form} name="dynamic_rule">
            ...
            <div className="row" hidden={twoFACodeVisible}>
              <Form.Item name="twofa">
        ...
        <p className="txt txt-center mt-24"><a onClick={onClickForgotPwd}>Forgot password?</a></p>
        <p className="txt txt-center mt-24"><a href='https://develop.fortrex.io'>Back home</a></p>
      </div>
    </div>
  );
};
