...
import {DEFAULT_REFERRAL_CODE, PAGE_PATHS} from 'constants/constant';
...
const RegisterForm = () => {
  ...
  const handleOnChange = (fieldName, item) => {
    switch (fieldName) {
      ...
      case 'confirmPass':
        ...
        break;
      case 'referralCode':
        setReferralCode(item.target.value);
        break;
      case 'fullName':
  ...
  const handleRegister = () => {
    try {
      ...
      const params = {
        ...
        ReferralCode: referralCode || DEFAULT_REFERRAL_CODE,
        FullName: fullName,
        UserName: userName,
      };
      register(params)
        .then(res => {
          ...
          if (res && res.data.StatusCode === 200) {
                history.push(PAGE_PATHS.SUCCESS_REGISTER);
  ...
  const handleOnChangeShowConfirmPassword = () => {
  ...
  return (
    <div>
      ...
      <p className="title-sub">Already have an account? <a onClick={onClickLogin}>Log in</a></p>
      <div className="login-form">
        <form>
          <div className="row">
            <input type="text" name="fullName" value={fullName} onChange={(item) => handleOnChange('fullName', item)}
                   placeholder="Enter full name"/>
          </div>
          <div className="row">
            <input type="text" name="userName" value={userName} onChange={(item) => handleOnChange('userName', item)}
                   placeholder="Enter user name"/>
          </div>
          <div className="row">
            <input type="text" name="email" value={email} onChange={(item) => handleOnChange('email', item)}
          ...
          <div className="row">
            <input type="text" name="code" disabled value={referralCode}
                   onChange={(item) => handleOnChange('referralCode', item)} placeholder="Referral code (optional)"/>
          </div>
          <Button style={{height: 45}} className={`btn warning ${loading ? 'disabled' : ''}`} disabled={loading}
                  name="submit" onClick={handleRegister}> REGISTER</Button>
        </form>
      </div>
      <p className="txt txt-center mt-24">By clicking Register button, you agree to <a href="">Term & Conditions</a></p>
    </div>
  );
};
